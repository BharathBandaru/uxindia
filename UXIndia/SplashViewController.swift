...
class SplashViewController: UIViewController {
    ...
    func show() {
        ...
        databaseRef.observeSingleEvent(of: .value, with: {(Snapshot) in
            ...
            ver = value?["vesionioscompare"]  as! Int
            if(ver == 1){
                if name == false{
                ...
                let alertController = UIAlertController(title: "Update", message: "Please update your app", preferredStyle: .alert)
    ...
    func addLogo() {
        ...
        let label = UILabel(frame: CGRect(x: (self.view.frame.size.width / 2), y: (self.view.frame.size.height / 2) + (h) - 20, width: 165, height: 21))
